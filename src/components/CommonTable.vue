<template>
    ...
    <slot name="top">
        <a-form style="min-height: 56px" :model="where" ref="form">
            <a-row>
                <a-col :span="16" style="display: flex; align-items: center; flex-wrap: wrap">
                    <template v-for="(field_name, index) in getFilterField(props.fields)" :key="index">
                        <a-form-item :label="fields[field_name].chineseName" style="margin-right: 10px" :name="field_name"
                            v-permission="['permission:query', permissions]">
                            <template v-if="fields[field_name].editComponent == 'ASelect'">
                                <component v-model:value="where[field_name]"
                                    :placeholder="'请选择' + fields[field_name].chineseName" :allowClear="true"
                                    :is="fields[field_name].editComponent" :fieldNames="{
                                        label: fields[field_name].componentOption.labelKey,
                                        value: fields[field_name].componentOption.valueKey,
                                    }" :mode="fields[field_name].componentOption.mode" :options="selectoptions[
    ...
    " style="width: 150px">
                            ...
                            <template v-else>
                                <component style="width: 150px" v-model:value="where[field_name]" :allowClear="true"
                ...
                <a-col :span="2" :offset="1" style="justify-content: space-evenly; display: flex">
                    <a-button type="primary" @click="emit('onQuery')" style="margin: 0 10px 0 0"
                        v-permission="['permission:query', permissions]">查询</a-button>
                    <a-button @click="handleReset" v-permission="['permission:reset', permissions]">重置</a-button>
                </a-col>
                <a-col :span="2" :offset="2">
                    <a-button type="primary" @click="emit('onInsert')"
                        v-permission="['permission:insert', permissions]">新建</a-button>
                </a-col>
            </a-row>
        </a-form>
    </slot>
    ...
    <!-- 表格 -->
    <a-table :columns="tableColumns" :data-source="tableData" :pagination="queryState" rowKey="objectId"
        :defaultExpandedRowKeys="expandedRowKeys" @change="emit('onChange', $event)" bordered
        ...
        <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.key === 'operation'">
                <div style="max-width: 230px; display: flex; align-items: center">
                    <template v-if="innerContainerTitle">
                        <a-button @click="emit('onAdd', record)" v-permission="['permission:insertChildren', permissions]">
                            {{ innerContainerTitle }}
                        </a-button>
                        <a-divider type="vertical" />
                    </template>
                    <a-button @click="emit('onEdit', record)" v-permission="['permission:edit', permissions]">
                        修改
                    </a-button>
                    <a-divider type="vertical" />
                    <a-popconfirm title="Are you sure delete this task?" ok-text="Yes" cancel-text="No"
                        @confirm="emit('onDelete', record)" @cancel="() => { }">
                        <a-button type="danger" v-permission="['permission:remove', permissions]">
                            删除
                        </a-button>
                    </a-popconfirm>
                </div>
            </template>
            <template v-else-if="column.key == 'icon' && fields[column.key].type == 'String'">
            ...
            <template v-else-if="fields[column.key].type == 'String' ||
        ...
        <template #expandIcon="{ record }">
            <div v-if="record.routes && record.routes.length">
                <button type="button" class="ant-table-row-expand-icon ant-table-row-expand-icon-collapsed"
                    aria-label="Expand row" v-show="expandedRowKeys.indexOf(record.objectId) === -1"
                    @click="expand(record.objectId)"></button>
                <button type="button" class="ant-table-row-expand-icon ant-table-row-expand-icon-expanded"
                    aria-label="Collapse row" v-show="expandedRowKeys.indexOf(record.objectId) !== -1"
                    @click="expand('')"></button>
            </div>
        </template>
    </a-table>
